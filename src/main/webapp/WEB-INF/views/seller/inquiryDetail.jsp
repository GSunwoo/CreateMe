	...
	<div class="mypage-wrapper">
		<div class="mypage-container">
			...
			<div class="mypage-content">
				...
				<div class="board-white-box">
					<table class="board-white-table">
						...
						<tbody>
							<tr>
								<th scope="row">상품명</th>
								<td><c:out value="${inquiry.prod_name}" /></td>
							</tr>
							<tr>
								<th scope="row">작성자</th>
				...
				<!-- ===== 판매자 답변 작성 ===== -->
				<form id="sellerReplyForm" action="${pageContext.request.contextPath}/seller/writeComment.do" method="post"
				      style="margin-top:12px">
				  <input type="hidden" name="inquiry_id" value="${inquiry.inquiry_id}">
				  <input type="hidden" name="prod_id"    value="${inquiry.prod_id}">
				  <textarea name="com_content" rows="6" style="width:90%" required placeholder="판매자 답변을 입력하세요"></textarea>
				...
				<div class="board-white-box" style="margin-top:20px">
				  <div class="mypage-zone-tit" style="border:none;margin:0 0 12px">
				    <h3 style="margin:0;font-size:1.2em">판매자 답변</h3>
				  </div>
				
				  <c:choose>
				    <c:when test="${not empty coms}">
				      <ul id="replyList" style="list-style:none;padding:0;margin:0">
				        <c:forEach var="ans" items="${coms}">
				          <li style="border:1px solid #eee;border-radius:6px;padding:12px 14px;margin-bottom:10px">
				            <div style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:6px">
				              <strong>판매자</strong>
				              <small><fmt:formatDate value="${ans.com_date}" pattern="yyyy-MM-dd HH:mm"/></small>
				            </div>
				            <div style="white-space:pre-line;line-height:1.5">${ans.com_content}</div>
				          </li>
				        </c:forEach>
				      </ul>
				    </c:when>
				    <c:otherwise>
				      <p id="noReply" style="color:#888;margin:8px 0 0">등록된 판매자 답변이 없습니다.</p>
				      <ul id="replyList" style="list-style:none;padding:0;margin:0"></ul>
				    </c:otherwise>
				  </c:choose>
				</div>
			</div>
