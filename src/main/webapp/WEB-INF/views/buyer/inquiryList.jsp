...
.mypage-table-type th, .mypage-table-type td {
	padding: 10px;
...
.btn-blue:hover {
  background-color: #075830; /* hover 시 더 진한 파란색 */
}
...
<body class="simple-page">
	...
	// 문의 리스트 받아오기
	List<com.farm.dto.InquiryDTO> inquiryList = (List<com.farm.dto.InquiryDTO>) request.getAttribute("inquiries");
				...
				<div class="mypage-info-cont">
							...
							<tbody>
								<c:choose>
									<c:when test="${not empty inquiries}">
										<c:forEach var="q" items="${inquiries}" begin="${startIndex}"
											end="${endIndex - 1}">
											<tr>
												<td><fmt:formatDate value="${q.postdate}"
												...
												<!-- 제목: 수정 페이지로 이동 (컨트롤러에 존재) -->
												<td class="ellipsis"><c:out value="${q.title}" /></td>
												<!-- 문의 상세보기 -->
												<td class="ellipsis">
												  <button type="button" class="btn-blue"
												          onclick="location.href='/inq/inquiryDetail.do?inquiry_id=${q.inquiry_id}'">
												    상세보기
												  </button>
												</td>
												<!-- 문의상태: 스키마가 없으니 고정 -->
												<!-- if, when  com_count 0이면 대기, 0보다 크면 답변완료 -->
												<td class="status">
												    <c:choose>
												        <c:when test="${q.com_count > 0}">
					...
					<div class="pagination">
						...
						<a href="?page=${totalPages}"
							class="${currentPage == totalPages ? 'disabled' : ''}">끝</a>
					</div>
				</div>
			</div>
		</div>
