...
public class MypageSellerController {
   ...
   public String sellerMypage(@AuthenticationPrincipal CustomUserDetails userDetails, Model model, ParameterDTO parameterDTO,
      ...
      if(userDetails!=null) {
         ...
         String[] items = {"양파", "마늘","파", "깻잎", "딸기", "배추", "상추", "수박", "참외",
        		 "토마토", "멜론", "방울토마토", "사과", "배", "포도", "감귤","복숭아"};
   ...
   public JSONObject sellerSaleData(@AuthenticationPrincipal CustomUserDetails userDetails){
    ...
    public ResponseEntity<Void> nextState(HttpServletRequest req, @RequestParam("purc_id") Long purc_id, @RequestParam("next") String next,
       ...
       if(next.equals("deli_order") || next.equals("cmpl_order")) {
    	   PassFindDTO passFindDTO =  mailDao.selectEmail(member_id);
    	   MailInfoDTO mailInfoDTO = new MailInfoDTO();
			// 메일 받을 주소
			mailInfoDTO.setTo(passFindDTO.getEmailid()+"@"+passFindDTO.getEmaildomain());
			// 메일 제목 지정
			
			mailInfoDTO.setSubject("[Farm To You] 주문상품 배송상태 안내");
			
			if (next.equals("deli_order")) {
				mailInfoDTO.setContent("[Farm To You]"+ passFindDTO.getName() + "님 주문하신 상품배송이 시작되었습니다." );				
			}
			else if(next.equals("cmpl_order")) {
				mailInfoDTO.setContent("[Farm To You]"+ passFindDTO.getName() + "님 주문하신 상품배송이 완료되었습니다." );
			}
			
			mailsender.deliverySender(mailInfoDTO);
       }
    ...
    public String member_info(@AuthenticationPrincipal CustomUserDetails ud,
    		Model model) {
    	MemberDTO memberDTO = ud.getMemberDTO();
    	model.addAttribute("memberDTO", memberDTO);
        return "seller/member-info";
    }
    @PostMapping("seller/updateMember")
    public String updateMember(MemberDTO memberDTO) {
    	int result = memfDao.updateMember(memberDTO);
    	if(result > 0) {
    		return "redirect:/seller/mypage.do";
    	}
    	else return "error : 사용자를 찾을수 없습니다.";
    }
}
